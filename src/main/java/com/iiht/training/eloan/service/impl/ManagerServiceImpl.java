...
import com.iiht.training.eloan.inspectors.InspectProcessinginfo;
import com.iiht.training.eloan.inspectors.InspectUserandLoandetails;
...
public class ManagerServiceImpl implements ManagerService {
	...
	private SanctionOutputDto convertToSanctionOutputDto(SanctionInfo sanctionInfo) {
		...
		InspectProcessinginfo validator = new InspectProcessinginfo();
        if (!validator.checkloanamountsanctioned(sanctionInfo))
            throw new InvalidDataException(validator.getErrors().toString());
	...
	private ProcessingDto convertToProcessingDto(ProcessingInfo processingInfo) {
		...
		InspectProcessinginfo validator = new InspectProcessinginfo();
        if (!validator.checkData(processingInfo))
            throw new InvalidDataException(validator.getErrors().toString());
	...
	private LoanOutputDto covertToLoanOutputDto(LoanDto loanDto, Loan newLoan) {
		...
		InspectUserandLoandetails validator = new InspectUserandLoandetails();
        if (!validator.isValidLoanInfo(loanDto))
            throw new InvalidDataException(validator.getErrors().toString());
